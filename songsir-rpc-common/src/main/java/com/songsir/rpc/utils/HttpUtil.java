...
public class HttpUtil {
    ...
    /**
     * @MethodName HttpPostForCuxiaoSt
     * @Description 只有调用实体促销时使用该方法，与上面不同的地方：a：异常捕获抛出；b: 增加连接时长
     * @Auther SongYapeng
     ...
     * @Since JDK 1.8
     */
    public static String HttpPostForCuxiaoSt(String url, String method, Map paramMap) throws Exception {
        String encoding = "UTF-8";
        String webUrl = url + "/" + method;
        log.info(webUrl);
        if (encoding == null || "".equals(encoding)) {
...
//			  	System.out.println(sBuffer);
    ...
    public static String jsonPostRequest(String url, Map<String, String> params) {
        ...
        try {
            ...
            jsonStr = postMethod.getResponseBodyAsString();
            if (log.isDebugEnabled()) {
                log.debug(jsonStr);
            }
            if (SongsirStringUtils.isEmpty(jsonStr)) {
                return null;
            }
        } catch (HttpException e) {
            //发生致命的异常，可能是协议不对或者返回的内容有问题
